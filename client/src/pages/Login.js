...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!email.trim() || !password.trim()) {
      setAlert({ type: 'error', msg: 'Please fill in both email and password.' });
      return;
    }
    try {
      setLoading(true);
      setAlert(null);
      const { data } = await axios.post('/users/login', { email: email.trim(), password });
      localStorage.setItem('user', JSON.stringify({ ...data.user, password: '' }));
      setAlert({ type: 'success', msg: 'Login successful! Taking you to the dashboard…' });
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* ── LEFT — 4K photo panel with Ken Burns ── */}
      <div className="auth-left-panel" style={{ '--auth-bg': "url('/auth-bg.png')" }}>
        <div className="auth-left-inner">
          ...
          {[
            { icon: '📊', title: 'Smart Analytics', desc: 'Visualise income vs expenses at a glance' },
            { icon: '💸', title: 'Multi-Currency', desc: 'INR, USD, EUR — switch anytime' },
            { icon: '📥', title: 'PDF Exports', desc: 'Download monthly statements instantly' },
          ].map(f => (
      ...
      <div className="auth-right-panel">
        <div style={{ width: '100%', maxWidth: 360 }}>
          ...
          <div style={{ marginBottom: 30 }}>
            ...
            <div style={{ fontSize: '1.6rem', fontWeight: 800, color: '#fff', lineHeight: 1.2 }}>Welcome back</div>
            <div style={{ fontSize: '0.86rem', color: 'rgba(255,255,255,0.48)', marginTop: 6 }}>Sign in to your account</div>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div style={{ marginBottom: 18 }}>
              ...
              <input
                ...
                onFocus={e => { e.target.style.borderColor = 'rgba(129,140,248,0.70)'; e.target.style.boxShadow = '0 0 0 4px rgba(99,102,241,0.14)'; }}
          ...
          <div style={{ textAlign: 'center', marginTop: 22, fontSize: '0.86rem' }}>
            <span style={{ color: 'rgba(255,255,255,0.40)' }}>Don't have an account? </span>
          ...
          <div style={{ textAlign: 'center', marginTop: 16, fontSize: '0.68rem', color: 'rgba(255,255,255,0.28)', lineHeight: 1.6 }}>
            By signing in, you agree to our <span style={{ color: 'rgba(255,255,255,0.45)', textDecoration: 'underline', cursor: 'pointer' }}>Terms &amp; Conditions</span>
